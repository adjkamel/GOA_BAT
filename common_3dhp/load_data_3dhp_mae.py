...
from common_3dhp.utils import deterministic_random
from common_3dhp.camera import world_to_camera, normalize_screen_coordinates
from common_3dhp.generator_3dhp import ChunkedGenerator
...
class Fusion(data.Dataset):
    def __init__(self, opt, root_path, train=True, MAE=False):
        ...
        if self.train:
            ...
            '''
            print(self.poses_train.get(('S1', 'Seq1', '0'))[1])



            
            x = self.poses_train.get(('S1', 'Seq1', '0'))[1][:,0]
            y = self.poses_train.get(('S1', 'Seq1', '0'))[1][:,1]
            z = self.poses_train.get(('S1', 'Seq1', '0'))[1][:,2]

            jts=list(range(1,18,1))
            ...
            print('INFO: Training on {} frames'.format(self.generator.num_frames()), len(self.poses_train))
        else:
