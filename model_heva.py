...
class AttentionC(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.norm2 = nn.LayerNorm(15*32)

    def forward(self, x):

       

        x_reshaped = x.view(-1, x.size(1), 15, 32)
        ...
        attention_scores = attention_scores.view(-1, x.size(1), 15)
        ...
        x_reshaped = x_reshaped.view(-1, x.size(1), 15, 32)
        ...
        x = attended_features.view(-1, x.size(1), 15 * 32)
...
class GOA_TF(nn.Module):
    def __init__(self, adj,num_frame=9, num_joints=15, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        self.conv1D = torch.nn.Conv1d(in_channels=5, out_channels=1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2)        #[256, 2, 9, 15]
        
        b, c, f, p = x.shape  ##### b is batch size, f is number of frames, p is number of joints
        
        x = rearrange(x, 'b c f p  -> (b f) p  c', )  #[2304, 15, 2]
        
        x = self.gconv_input(x)
        
        x = rearrange(x, '(b f) p c -> b f (p c)', f=f)  #[64, 459, 32]
        
        #x += self.Temporal_pos_embed     #[256,9,544] 
